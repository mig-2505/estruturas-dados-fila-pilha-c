...
#define STR 50    // tamanho máximo da descrição da operação

// Estrutura da Pilha
typedef struct {
    char itens[MAX][STR]; // cada operação é uma string
    int topo;
} Pilha;
...
void push(Pilha *p, char operacao[]) {
    if (cheia(p)) {
        printf("⚠️ Pilha cheia! Não é possível registrar mais operações.\n");
    } else {
        p->topo++;
